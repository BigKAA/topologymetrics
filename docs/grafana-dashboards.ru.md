...
## Рекомендуемые панели для метрик статуса

Начиная с v0.4.0, dephealth экспортирует две дополнительные метрики: `app_dependency_status` (категория статуса) и `app_dependency_status_detail` (детальная причина сбоя). Текущие дашборды используют `app_dependency_health` и `app_dependency_latency_seconds`. Следующие панели можно добавить для улучшения видимости причин сбоев.

### Панель категории статуса (Table)

Показывает текущую категорию статуса для каждой зависимости. Добавьте в **Service Status** или **Link Status** дашборды.

```promql
# Текущая категория статуса (серия со значением = 1)
app_dependency_status{status!=""} == 1
```

Отобразите как Table-панель с колонками: dependency, host, port, status (значение метки). Используйте value mappings для цветового кодирования:

| Статус | Цвет | Значение |
| --- | --- | --- |
| `healthy` | зелёный | Зависимость здорова |
| `connection_error` | красный | Невозможно установить соединение |
| `timeout` | оранжевый | Таймаут соединения или ответа |
| `auth_error` | красный | Ошибка аутентификации/авторизации |
| `response_error` | оранжевый | Неожиданный ответ (напр. HTTP 5xx) |
| `protocol_error` | оранжевый | Ошибка на уровне протокола |
| `resource_error` | оранжевый | Исчерпание ресурсов |
| `unknown_error` | серый | Неклассифицированная ошибка |

### Панель детальной причины (Table)

Показывает детальную причину сбоя вместе с категорией статуса. Добавьте в **Link Status** дашборд для детального расследования.

```promql
# Детальная причина (непустая метка detail при значении = 1)
app_dependency_status_detail{detail!=""} == 1
```

Отобразите как Table-панель с колонками: dependency, host, port, detail. Это помогает операторам быстро определить точную причину сбоя (напр. `connection_refused`, `http_503`, `password_authentication_failed`).

### Панель timeline статусов

Показывает изменения категории статуса во времени. Добавьте в **Service Status** дашборд рядом с существующим timeline здоровья.

```promql
# Категория статуса во времени
label_replace(
...
Отобразите как State Timeline панель, маппируя значения метки status на цвета.
