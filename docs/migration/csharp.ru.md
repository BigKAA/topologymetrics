...
Ключевые особенности: потокобезопасность (lock + ConcurrentDictionary),
идемпотентность add/remove, наследование глобальной конфигурации, полная
очистка метрик при remove/update.

Новое исключение: `EndpointNotFoundException` (выбрасывается `UpdateEndpoint`,
если старый эндпоинт не найден).

Подробности: [руководство по миграции v0.5.0 → v0.6.0](sdk-csharp-v050-to-v060.ru.md).
