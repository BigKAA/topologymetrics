...
- [x] 24.1. Рефакторинг `run.sh` — поддержка Helm-деплоя
- [x] 24.2. Параметр `--namespace` в `verify.py` и `utils.py`
- [x] 24.3. Обновить `conformance/runner/Dockerfile` — добавить `cross_verify.py`
- [x] 24.4. Создать `conformance/README.md` — описать оба режима
- [ ] 24.5. Прогнать conformance с Helm-деплоем (вручную)
...
- [x] 25.1. Создать `CONTRIBUTING.md` в корне проекта
- [x] 25.2. Обновить корневой `README.md` — ссылка на CONTRIBUTING
- [ ] 25.3. Обновить `CLAUDE.md` — не требуется (уже актуален)
- [ ] 25.4. Обновить `docs/quickstart/*.md` — не требуется (уже generic)
