...
мониторируемого эндпоинта зависимости через Micrometer. Это руководство
...
| `app_dependency_latency_seconds` | DistributionSummary | Латентность проверки в секундах |
...
Пользовательские метки, добавленные через `.label()`, появляются
...
Distribution summary латентности проверок с гистограммными бакетами.
...
Метрики доступны автоматически на `/actuator/prometheus`. Убедитесь,
что эндпоинт открыт:
...
Экспорт через Micrometer `PrometheusMeterRegistry`:

```java
import io.micrometer.prometheus.PrometheusMeterRegistry;

// В HTTP-обработчике для /metrics:
