*[English version](config-contract.md)*
...
- `group` (обязательный) — логическая группа сервиса (команда, подсистема, проект).
  Формат: `[a-z][a-z0-9-]*`, длина 1-63 символа. Альтернативно задаётся через
  `DEPHEALTH_GROUP` (API > env var). При отсутствии и в API, и в env — ошибка конфигурации.
...
| Отсутствует группа | `missing group` |
| Невалидная группа | `invalid group: "..."` |
...
| `DEPHEALTH_GROUP` | Логическая группа (метка `group`) | `DEPHEALTH_GROUP=billing-team` |
