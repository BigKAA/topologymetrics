...
dependency endpoint via prometheus-net. This guide describes each metric,
...
Custom labels added via `labels: new Dictionary<string, string> { ... }` appear
after `critical` in alphabetical order.
...
Metrics are exposed at `/metrics` via `prometheus-net.AspNetCore`. Add
`app.MapMetrics()` to the request pipeline:
...
Ensure `prometheus-net.AspNetCore` is installed:
...
When not using ASP.NET Core, use the `MetricServer` from prometheus-net
or expose metrics from a custom HTTP handler via `Metrics.DefaultRegistry`:

```csharp
using Prometheus;

// Standalone metric server on port 9090
...
// In a custom HTTP handler -- scrape the default registry
