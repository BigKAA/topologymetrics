...
dependency endpoint. This guide describes each metric, its labels,
and provides PromQL examples.
...
| `app_dependency_latency_seconds` | Histogram | Check latency in seconds |
...
| --- | --- | --- |
| `name` | `New()` first arg | Application name |
| `group` | `New()` second arg | Logical group |
| `dependency` | Dependency name | Dependency identifier |
| `type` | Checker type | `http`, `grpc`, `tcp`, `postgres`, `mysql`, `redis`, `amqp`, `kafka` |
| `host` | `FromURL`/`FromParams` | Dependency host |
| `port` | `FromURL`/`FromParams` | Dependency port |
| `critical` | `Critical()` | `yes` or `no` |

Custom labels added via `WithLabel()` appear after `critical` in
alphabetical order.
...
| `auth_error` | HTTP, gRPC, PG, MySQL, Redis, AMQP | Auth failure |
...
gets value `0` and the new one gets `1`.
...
By default, metrics are registered with `prometheus.DefaultRegisterer`.
...
- Separate registry for testing
- Push gateway integration
- Multiple `DepHealth` instances (each needs its own registerer to avoid
